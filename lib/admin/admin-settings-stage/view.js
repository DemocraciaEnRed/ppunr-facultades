...
export default class AdminSettings extends FormView {
  ...
  cleanProyectistas(){
    this.clearProjectistBtn = document.getElementById('borrar-proyectistas')
    this.clearProjectistBtn.addEventListener('click',clearProjectistFunc)

    function clearProjectistFunc(){
      confirm({
        text: '¿desea borrar los proyectos, votos y proyectistas?',
        acceptText: 'borrar'
      }).accepts(() => {
        console.log('accept');
      })
