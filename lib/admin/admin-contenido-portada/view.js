  ...
  dosomething(){
    ...
    request
      ...
      .end((err, res) => {
        ...
        if (Object.keys(res.body).length === 0) this.find('pre#response-dni')[0].textContent = 'No se a encontrado nadie con el DNI ' + el.value()
        else this.find('pre#response-dni')[0].textContent = JSON.stringify(res.body, null, " ")
    ...
    submitPadron (){
      ...
      request
      .post('/api/padron/new')
      .send({ dni: elDNI.value(), escuela: elEscuela.value() })
      .end((err, res) => {
        if (err || !res.ok) {
        ...
        elDNI.value('')
        this.find('#success-new-dni')[0].textContent = 'OK! Agregado el DNI ' + theDni + ' a la escuela ' + this.locals.escuelas.find(e => e.value == theEscuela).name
