...
export default class ForumTagsSearch extends Component {
  ...
  inputMount = (el) => {
    ...
    input.addEventListener('keyup', this.searchTags, false)
  ...
  searchTags = (evt) => {
    ...
    const tags = input.value ? this.state.allTags.filter((t) => t.toLowerCase().startsWith(input.value.toLowerCase())) : []
    if (tags.length < 1){
      const fuzzyTags = input.value ? this.state.allTags.filter((t) => fuzzysearch(input.value, t)) : []
      fuzzyTags.forEach(e => !tags.includes(e) && tags.push(e))
    }
