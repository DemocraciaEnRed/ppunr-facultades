...
page(
  ...
  findPrivateTopics,
  // cargamos escuelas, claustros y tags en ctx.uniqAttrs para los filtros
  (ctx, next) => {
    ...
    Promise.all([
      ...
      const [escuelas, claustros, tags] = results
      ctx.uniqAttrs = {
        escuelas: escuelas.map(escuela => { return {value: escuela._id, name: escuela.nombre}; }),
      ...
      log('Unable to load escuelas, claustros and tags.')
    })
  },
  (ctx) => {
...
page(
  ...
  (ctx) => {
    const contenidoPortada = new ContenidoPortada(ctx.uniqAttrs.escuelas, ctx.uniqAttrs.claustros)
