...
import DownloadOlds from './download-old/component'
...
export default class TopicsListView extends View {
  ...
  switchOn () {
    this.bind('click','.delete-topics', this.bound('onDeteleClick'))
    ...
    ReactRender((<DownloadOlds
      forum={this.forum}
    />), this.el[0].querySelector('.download-old-topics-wrapper'))
  ...
  onDeteleClick(){
    const onconfirmdelete = (ok) => {
      if (!ok) return
      topicStore.deleteAllTopics(this.forum).then(results=>{
    ...
    confirm('¿Desea eliminar proyectos e ideas?')
    .cancel('cancelar')
    .ok('aceptar')
    .modal()
    .closable()
  ...
  loadFiltersFromUrl(){
    ...
    let facultad = getQueryVariable('facultad')
    if (facultad) this.el[0].querySelector('select[name=facultad]').value = decodeURIComponent(facultad)
