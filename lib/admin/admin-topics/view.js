...
export default class TopicsListView extends View {
  ...
  onbuscarclick (ev){
    ...
    let claustroSelect = this.el[0].querySelector('select[name=claustro]')
    if (claustroSelect.value) query.claustro = claustroSelect.value

    let escuelaSelect = this.el[0].querySelector('select[name=escuela]')
    if (escuelaSelect.value) query.escuela = escuelaSelect.value

    // let tipoSelect = this.el[0].querySelector('select[name=tipo]')
    // if (tipoSelect.value) query.tipo = tipoSelect.value
  ...
  loadFiltersFromUrl(){
    ...
    if (search) this.el[0].querySelector('input[name=busqueda]').value = decodeURIComponent(search)

    let escuela = getQueryVariable('escuela')
    if (escuela) this.el[0].querySelector('select[name=escuela]').value = decodeURIComponent(escuela)

    let claustro = getQueryVariable('claustro')
    if (claustro) this.el[0].querySelector('select[name=claustro]').value = decodeURIComponent(claustro)

    let tipo = getQueryVariable('tipo')
    if (tipo) this.el[0].querySelector('select[name=tipo]').value = decodeURIComponent(tipo)
