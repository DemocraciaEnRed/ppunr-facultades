...
export default class TopicsListView extends View {
  ...
  loadFiltersFromUrl(){
    ...
    if (anio) this.el[0].querySelector('select[name=anio]').value = decodeURIComponent(anio)

    let barrio = getQueryVariable('barrio')
    if (barrio) this.el[0].querySelector('select[name=barrio]').value = decodeURIComponent(barrio)
