...
export function findPrivateTopics (ctx, next) {
  ...
  let query = {
    ...
    escuela: getQueryVariable('escuela') || '',
...
export function findUniqTopicAttrs (...attrs) {
  return function (ctx, next) {
   topicStore.findUniqTopicAttrs(attrs).then(attrVals => {
     ctx.uniqAttrs = {}
     attrs.forEach((attr, i) => {
       ctx.uniqAttrs[attr] = attrVals[i]
     })
     next()
   }).catch((err) => {
     if (err.status !== 404) throw err
     log('Unable to load uniq topic attrs.')
   })
  }
}
