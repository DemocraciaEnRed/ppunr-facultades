...
exports.getCount = () => {
  log('Counting topics not deleted')
  return Topic.count({ deletedAt: null }).count()
}

exports.sumProyectistasInTopics = () => {
  // Count all the items in topic.proyectistas array and sum the total in all the topics not deleted
  log('Counting proyectistas in topics not deleted')
  return Topic.aggregate([
    { $unwind: '$proyectistas' },
    { $group: { _id: '#proyectistas', total: { $sum: 1 } } }
  ]).then(res => {
    if (!res[0]) return 0
    return res[0].total
  })
...
exports.getCommentsCount = () => {
  log('Counting comments not deleted')
  return Comment.find({ deletedAt: null }).count()
}

exports.sumRepliesInComments = () => {
  // Count all the items in comment.replies array and sum the total in all the not deleted comments
  log('Counting replies in comments not deleted')
  return Comment.aggregate([
    // keep only comments not deleted
