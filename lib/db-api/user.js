...
exports.populateUser = function populateUser(user){
  return Promise.all([
    api.facultad.get(user.facultad),
    api.claustro.get(user.claustro)
  ]).then((results) => {
    const [ facultad, claustro ] = results
...
exports.populateOwners = function populateOwners(topics){
  return Promise.all(topics.map(topic =>
    exports.populateUser(topic.owner)
  )).then(() => topics)
}
