...
exports.populateUser = function populateUser(user, withLastName){
  return Promise.all([
    api.escuela.getArray(user.escuelas),
    api.claustro.get(user.claustro),
    withLastName ? User.findOne({ _id: user.id }).select('lastName') : undefined
  ]).then((results) => {
    const [ escuelas, claustro, userData ] = results
    user.escuelas = escuelas
...
exports.populateOwners = function populateOwners(topics, withLastName){
  return Promise.all(topics.map(topic =>
    ...
    exports.populateUser(topic.owner, true)
  )).then(() => topics)
}
...
exports.getFullUserById = function populateUser(userId, withLastName){
  return User.findOne({ _id: userId }).select(
    ...
    const [ user, escuelas, claustro ] = results
    user.escuelas = escuelas
    user.claustro = claustro
    return user
  })
