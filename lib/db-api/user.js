...
exports.getAllUsersInDNIArray = (dniArray) => {
  ...
  return User.find({ dni: { $in: dniArray }, emailValidated: true }).populate('claustro facultad')
}
...
exports.getUsersWhoDidntVoted = (dniArray) => {
  ...
  return User.find({ dni: { $nin: dniArray }, emailValidated: true })
}
exports.getUsersWhoDidntVotedWithClaustroAndFacultad = (dniArray) => {
  log('Looking for users who didnt voted')
  return User.find({ dni: { $nin: dniArray }, emailValidated: true }).populate('claustro').populate('facultad')
}
