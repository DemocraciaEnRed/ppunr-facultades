...
app.post('/settings/profile', restrict, function (req, res, next) {
  ...
  api.padron.isInPadron(proposedDni).then(padron => {
    if (padron) {
      ...
      return res.status(200).json({ error: 'Tu DNI no se encuentra en el padrón. Revisá si tu DNI es el correcto o completá <a href="https://docs.google.com/forms/d/e/1FAIpQLScc-ipTeJQxzwE-l8wWT3RVjsYPj-fmIy7Z7yggsFrbyKVjZw/viewform" target="_blank">este formulario</a> con tus datos' })
  })
}, (req, res) => {
  var user = req.user
  const proposedDni = req.body.dni
  ...
  // save in boolean
  if (req.body.proyectista) {
    user.proyectista = req.body.proyectista === 'true'
  }
  if (proposedDni)
...
app.post('/settings/proyectistas/join', restrict, function (req, res, next) {
  var user = req.user
  log('New proyectista, user %s', user.id)
