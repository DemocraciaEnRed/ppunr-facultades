    ...
    if (padron){
      ...
      signup.doSignUp(profile, meta, function (err, data) {
        ...
        if (config.env === 'development')
          api.padron.updateUserId(profile.dni, data.user ? data.user._id : data._id).then(() =>
            res.status(200).send()
          )
        else
      ...
      api.escuela.get(profile.escuela).then(escuela => {
        ...
        return res.status(200).json({
          error: `Tu DNI no se encuentra en el padrón. Revisá si tu DNI es el correcto o escribinos a presupuestoparticipativo@unr.edu.ar completando los siguientes datos: DNI, claustro y Escuela.`
        })
      })
  })
})
