...
exports.findAllFromForum = function findAllFromForum (req, res, next) {
  ...
  api.topics.find({ forum: id }).populate('tag')
    .exec()
    .then((topics) => {
      if (!topics) {
...
exports.findDeletedFromForum = function findAllFromForum (req, res, next) {
  const id = req.forum.id
  let query = {
    'attrs.state': { '$in': req.query.type === 'todos' ? ['pendiente', 'proyecto' ] : [req.query.type] }
  }


  api.topics.findDeleted({ forum: id }).populate('tag')
    .where(query)
    .exec()
    .then((topics) => {
      if (!topics) {
...
exports.parseUpdateableKeys = function parseUpdateableKeys (req, res, next) {
  // los attrs. aceptados vienen del campo topicsAttrs del forum principal
