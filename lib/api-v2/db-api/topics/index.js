...
exports.list = function list (opts) {
  ...
  return getPossibleOwners(opts).then(owners => {
    // si devuelve null es porque no se filtró por owner/claustro
...
/**
 * Get years of all deleted topics
 *
 *
 * @return {promise}
 * @api public
 */
exports.getYearsDeletedTopics = function getYearsDeletedTopics(){
  return findDeleted()
    .exec()
    .then(topics=>{ 
      let listYears = topics.map(topic=>topic.createdAt.getFullYear())
      return listYears.filter((item,index)=>{
        return listYears.indexOf(item) === index;
      });
        })
}
...
exports.deleteAll = function deleteAll (opts){
  return getPossibleOwners(opts).then(owners => {
    // si devuelve null es porque no se filtró por owner/claustro
    if (owners === null || owners.length > 0){
      const query = { forum: opts.forum }
      ...
      return find()
        ...
        .then((topics)=>{
          return topics.map(function (topic){
            setAttributes({ deletedAt: new Date() }, topic)
            return topic;
          })
        })
        .then((topics) => {
          topics.forEach(topic => topic.save())
          return topics
        })
      } else
        return 0
...
exports.vote = function vote (opts) {
  ...
  const dni = opts.dni
  const facultad = opts.facultad
  const claustro = opts.claustro
  ...
  return find()
    ...
    .populate(scopes.ordinary.populate)
    .exec()
    .then(doVote.bind(null, user, dni, value, facultad, claustro, votacionAbierta))
    .then((topic) => scopes.ordinary.expose(topic, forum, user))
}
...
function doVote (user, dni, value, facultad, claustro, votacionAbierta, topic) {
  ...
  return votedBy(user, topic, dni)
    .then((voted) => {
     ...
     if (value == 'voto'){
       ...
       return Vote.find({ value, dni: dni }).then(voto => {
