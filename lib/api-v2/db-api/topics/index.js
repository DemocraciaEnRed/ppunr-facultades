...
const getPossibleOwners = (opts) => {
  if (opts.claustro)
    return apiV1.user.findIds({claustro: { $in: opts.claustro }})
  else
...
exports.list = function list (opts) {
  ...
  return getPossibleOwners(opts).then(owners => {
    ...
    if (owners === null || owners.length > 0){
      ...
      if (opts.escuela) query.escuela = opts.escuela
  ...
  return votedBy(user, topic)
    .then((voted) => {
     ...
     if (value == 'voto'){
       ...
       const topicEscuelaId = topic.escuela._id.toString()
       if (!user.escuelas || !user.escuelas.find(escuelaId => escuelaId.toString() == topicEscuelaId)){
         const err = new Error('No podés votar en esta escuela')
         err.code = 'NOT_IN_ESCUELA'
         throw err
       }

       // si el vote es de votación solo permitir uno por usuarix en todo el sistema
       return Vote.findOne({value, author: user}).then(voto => {
         if (voto){
           const err = new Error('You already voted a topic.')
           err.code = 'ALREADY_VOTED_ONE'
           throw err
         }
       })
     }
