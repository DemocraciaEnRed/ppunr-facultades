    ...
    .then((topicsComments) => {
          ...
          (comment.replies.length === 0 ? [emptyReply] : comment.replies)
            .forEach((reply) => {
              commentsData.push([
                ...
                `"${escapeTxt(comment.author.claustro.nombre)}"`,
                ...
                `"${escapeTxt(reply.author.claustro.nombre)}"`,
      ...
      json2csv(commentsData, function (err, csv) {
        ...
        res.set({
          ...
          'Content-Disposition': 'attachment; filename=comentarios-facultades.csv'
        })
