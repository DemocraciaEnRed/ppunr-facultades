...
app.get('/topics/find-all-proyectos', (req, res, next) => {
  apiV1.topic.search({'attrs.state': 'proyecto', escuela: req.query.escuelaId}, (err, topics) => res.status(200).json(topics))
})
...
app.post('/topics',
middlewares.users.restrict,
middlewares.users.restrictEscuelasFromBody,
...
function postTopics (req, res, next) {
  if (config.propuestasAbiertas || req.user.staff || req.forum.hasRole(req.user, 'admin', 'collaborator', 'author')){
    ...
    if (req.keysToUpdate['attrs.state'] === 'sistematizada'){
      if (req.keysToUpdate['attrs.escuela'] !== 'ninguna')
        req.keysToUpdate['escuela'] = req.keysToUpdate['attrs.escuela']
    }
    // delete req.keysToUpdate['attrs.escuela']
...
middlewares.users.restrict,
middlewares.topics.findById,
middlewares.users.restrictEscuelasFromTopic,
middlewares.forums.findFromTopic,
middlewares.forums.privileges.canCreateTopics,
